...
const existingCharacterPages = new Set(["potpourri", "rea", "sophia", "veela", "sivi", "cerberus", "aa_dark", "cattleya"]);
...
function summarizeEffectTags(row) {
  const text = [row.multiplierText, row.duration, row.condition, row.exclusiveWeapon].filter(Boolean).join(" ");
  const tagRules = [
    ["HP回復", "HP回復"], ["再生", "再生"], ["シールド", "シールド"], ["ステルス", "ステルス"],
    ["被ダメージ増加", "被ダメ増"], ["被ダメージ減少", "被ダメ減"], ["魔法防御力減少", "魔防減"],
    ["物理防御力減少", "物防減"], ["防御力減少", "防御減"], ["回避率減少", "回避減"],
    ["命中率減少", "命中減"], ["攻撃力増加", "攻撃増"], ["攻撃力減少", "攻撃減"],
    ["クリティカル率増加", "クリ率増"], ["クリティカル率減少", "クリ率減"], ["最大HP増加", "最大HP増"],
    ["HPドレイン増加", "HPドレイン増"], ["直接攻撃", "直接攻撃"], ["気絶", "気絶"], ["沈黙", "沈黙"],
    ["遅延", "遅延"], ["脱力", "脱力"], ["毒", "毒"], ["浸食", "浸食"], ["バフ無効", "バフ無効"],
    ["解除不可", "解除不可"], ["必ず命中", "必中"], ["再発動", "再発動"], ["スキルCT", "CT操作"]
  ];
  const tags = tagRules.filter(([keyword]) => text.includes(keyword)).map(([, label]) => label);
  return tags.length ? tags.join(" / ") : "追加効果";
}

function displaySubtype(row) {
  if (String(row.subtype || "").includes("画像記載効果")) return summarizeEffectTags(row);
  return row.subtype;
}

function displayValue(value) { return valueOrDash(normalizeTurnText(value)); }
function normalizeRankingTarget(value) { return value === "対象" ? "基本" : value; }

function getDamageValue(row, key = state.sort) {
  if (!row.damage) return null;
  const damageKeyMap = { base: "baseTotal", lv1: "exclusiveLv1Total", lv2: "exclusiveLv2Total", lv3: "exclusiveLv3Total", max: "conditionMaxTotal" };
...
function formatDamageStages(row) {
  ...
  stages.forEach((stage) => {
    ...
    if (last && last.value === stage.value) last.labels.push(stage.label);
    else grouped.push({ labels: [stage.label], value: stage.value });
  });
  const parts = grouped.map((group) => {
    const label = group.labels.length === 1 ? group.labels[0] : `${group.labels[0]}-${group.labels[group.labels.length - 1]}`;
...
function renderDefinitionPairs(pairs) {
  return pairs.filter(([, value]) => value !== "" && value !== null && value !== undefined).map(([key, value]) => `<div><dt>${key}</dt><dd>${displayValue(value)}</dd></div>`).join("");
}
...
function normalizeSkill(rawSkill, character) {
  ...
  if (rawSkill.id === "luke-s2" && !damage) {
    damage = { singleMultiplier: 520, hitCount: 1, baseTotal: 520, exclusiveLv1Total: 520, exclusiveLv2Total: 520, exclusiveLv3Total: 520, conditionMaxTotal: 780, conditionMaxNote: "対象がシールドなしなら直接攻撃ダメージ1.5倍", powerType: "直接攻撃", maxStacks: null, realityRank: "中", rankingTarget: "条件込み", damageMemo: "腕力参照の直接攻撃。通常の物理/魔法倍率とは別枠" };
  }
  if (damage) {
  ...
  return {
    ...
    memo: compareMemo,
    weaponPassive,
    source: character?.pageSlug && existingCharacterPages.has(character.pageSlug) ? `./pages/characters/${character.pageSlug}.html` : rawSkill.sourceUrl || character?.sourceUrl || "#",
...
function skillMode(skill) { return skill.skillType === "パッシブ" ? "passive" : "active"; }
...
function buildDatasets(data) {
  ...
  datasets.active = skills.filter((skill) => skillMode(skill) === "active").map((skill) => ({ ...skill, mode: "active" }));
  datasets.passive = skills.filter((skill) => skillMode(skill) === "passive").map((skill) => ({ ...skill, mode: "passive" }));
}
...
function currentRows() {
  ...
  if (query) {
    rows = rows.filter((row) => [row.character, row.skill, row.skillType, row.category, row.subtype, row.target, row.multiplierText, row.condition, row.exclusiveWeapon, row.memo, row.weaponPassive].filter(Boolean).some((value) => String(value).toLowerCase().includes(query)));
  }
  rows.sort((a, b) => {
    if (state.mode === "active") {
    ...
    if ((a.character || "") !== (b.character || "")) return (a.character || "").localeCompare(b.character || "", "ja");
...
function renderHeader() {
  const headers = state.mode === "active" ? ["キャラ", "スキル", "種別", "対象", "CT", "効果", "火力", "条件・追加効果", "専用武器"] : ["キャラ", "スキル", "種別", "対象", "CT", "効果", "条件・追加効果", "専用武器"];
...
function renderRows(rows) {
  if (rows.length === 0) {
    ...
    el.tableBody.innerHTML = `<tr><td class="empty-state" colspan="${colspan}"><strong>該当するスキルがありません</strong>属性や検索条件を少し広げてください。</td></tr>`;
    return;
  }
  el.tableBody.innerHTML = rows.map((row) => {
    ...
    return `<tr data-id="${row.id}"${selected}><td><label class="row-check"><input type="checkbox" data-pin-id="${row.id}"${checked} aria-label="${row.character} ${row.skill}を比較候補に残す">${characterCell(row)}</label></td><td><strong>${row.skill}</strong><br><span class="pill">スキル${row.skillNumber}</span></td><td><span class="pill">${displayValue(row.category)}</span><br>${displayValue(displaySubtype(row))}</td><td class="target-cell">${displayValue(row.target)}</td><td>${displayValue(row.ct)}</td><td>${displayValue(row.multiplierText)}</td>${damageCell}<td class="condition-cell">${displayValue(row.condition || row.duration)}</td><td class="condition-cell">${displayValue(row.exclusiveWeapon)}</td></tr>`;
  }).join("");
}
...
function renderBoard() {
  ...
  el.compareBoard.innerHTML = rows.map((row) => `<article class="board-card"><div class="board-card-head"><span class="mini-avatar ${attrClass(row.attribute)}">${getInitial(row.character)}</span><div><strong>${row.character}</strong><span>${row.skill}</span></div></div><dl class="board-note">${renderDefinitionPairs([["種別", `${displayValue(row.skillType)} / ${displayValue(displaySubtype(row))}`], ["対象", row.target], ["効果", row.multiplierText], ["表示火力", state.mode === "active" ? formatSelectedDamage(row) : ""], ["火力目安", formatDamageStages(row)], ["現実性", row.damage?.realityRank], ["ランキング対象", normalizeRankingTarget(row.damage?.rankingTarget)], ["条件・追加効果", row.condition || row.duration], ["専用武器", row.exclusiveWeapon], ["専用武器パッシブ", row.weaponPassive], ["比較メモ", row.memo]])}</dl></article>`).join("");
}
...
function renderDetail(row) {
  ...
  el.detailList.innerHTML = renderDefinitionPairs([["対象", row.target], ["CT", valueOrDash(row.ct)], ["効果", row.multiplierText], ["表示火力", state.mode === "active" ? formatSelectedDamage(row) : ""], ["火力目安", formatDamageStages(row)], ["現実性", row.damage?.realityRank], ["ランキング対象", normalizeRankingTarget(row.damage?.rankingTarget)], ["持続", row.duration], ["条件・追加効果", row.condition], ["専用武器", row.exclusiveWeapon], ["専用武器パッシブ", row.weaponPassive], ["比較メモ", row.memo]]);
}
...
function updateModeUi() {
  ...
  document.querySelectorAll(".metric-only").forEach((node) => { node.style.display = state.mode === "active" ? "" : "none"; });
}
...
function bindEvents() {
  ...
  el.realityFilters.forEach((checkbox) => checkbox.addEventListener("change", () => { checkbox.checked ? state.reality.add(checkbox.value) : state.reality.delete(checkbox.value); state.selectedId = null; render(); }));
  el.rankTargetFilters.forEach((checkbox) => checkbox.addEventListener("change", () => { checkbox.checked ? state.rankTarget.add(checkbox.value) : state.rankTarget.delete(checkbox.value); state.selectedId = null; render(); }));
  ...
  const resetFilters = () => { state.attribute = "all"; state.search = ""; state.sort = "base"; state.reality = new Set(["高", "中", "低"]); state.rankTarget = new Set(["基本", "条件込み"]); state.selectedId = null; el.search.value = ""; el.sort.value = "base"; el.realityFilters.forEach((checkbox) => { checkbox.checked = state.reality.has(checkbox.value); }); el.rankTargetFilters.forEach((checkbox) => { checkbox.checked = state.rankTarget.has(checkbox.value); }); el.attributeFilter.querySelectorAll("button").forEach((button) => { button.classList.toggle("active", button.dataset.value === "all"); }); render(); };
  el.resetButtons.forEach((button) => button.addEventListener("click", resetFilters));
}
